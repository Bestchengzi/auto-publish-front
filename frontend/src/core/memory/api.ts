...
export async function exportMemory(): Promise<UserMemory> {
  const response = await backendFetch("/api/memory/export");
...
export async function importMemory(memory: UserMemory): Promise<UserMemory> {
  const response = await backendFetch("/api/memory/import", {
    method: "POST",
    headers: {
...
export async function createMemoryFact(
  ...
  const response = await backendFetch("/api/memory/facts", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
