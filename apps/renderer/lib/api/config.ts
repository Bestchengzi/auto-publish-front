 ...
 * API 配置
 *
 * - 通用后端：使用同源前缀 `/baseApi`（见 `getApiBaseUrl`），由 Next `rewrites` 转发到实际服务，不依赖环境变量。
 * - LangGraph：生产环境走同源 `/baseApi` 代理（`/baseApi` + {@link LANGGRAPH_API_PREFIX}）。开发环境若设置
 ...
 *
 * 生产环境经 Nginx 反代流式输出时：务必对 `/baseApi/` 关闭 `proxy_buffering`、拉长读超时，示例见
...
/** LangGraph API 根路径（相对站点根），经 `/baseApi` 转发到后端（生产或与 dev 环境变量联用时）。 */
 ...
 * - 生产：`window.location.origin` + {@link API_BASE_URL} + {@link LANGGRAPH_API_PREFIX}，经同源 `/baseApi` 代理。
 * - 生产且无 `window` 时返回空字符串。
 */
