...
const NEED_HELP_SELECTION_RE =
  /\[NEED_HELP_SELECTION\][\s\S]*?\[\/NEED_HELP_SELECTION\]/g;

/**
 * Replace the structured "need help selection" marker block with a human-readable
 * summary so the corresponding human message isn't rendered as empty.
 */
export function stripNeedHelpSelectionMarker(content: string): string {
  return content
    .replace(NEED_HELP_SELECTION_RE, (match) => {
      const start = "[NEED_HELP_SELECTION]";
      const end = "[/NEED_HELP_SELECTION]";
      const s = match.indexOf(start);
      const e = match.lastIndexOf(end);
      if (s === -1 || e === -1 || e <= s) {
        return "";
      }

      const block = match.slice(s + start.length, e).trim();
      const lines = block
        .split("\n")
        .map((l) => l.trim())
        .filter(Boolean);
      ...
      for (const line of lines) {
        ...
        if (supplementMatch) {
          supplement = supplementMatch[1]?.trim() ?? "";
          continue;
        }
