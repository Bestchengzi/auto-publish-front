...
import { Loader } from "@/components/langgraph/ai-elements/loader";
import {
  Message as AIElementMessage,
  MessageContent as AIElementMessageContent,
  MessageResponse as AIElementMessageResponse,
  MessageToolbar,
} from "@/components/langgraph/ai-elements/message";
import {
  Reasoning,
  ReasoningContent,
  ReasoningTrigger,
} from "@/components/langgraph/ai-elements/reasoning";
import { Task, TaskTrigger } from "@/components/langgraph/ai-elements/task";
...
import { humanMessagePlugins } from "@/lib/langgraph/core/streamdown";
...
export function MessageListItem({
  ...
  return (
    ...
    >
      <MessageContent
        className={isHuman ? "w-[64%] max-w-[64%] min-w-0" : "w-full"}
        message={message}
        isLoading={isLoading}
      />
      {!isLoading && (
...
function MessageImage({
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer">
      {/* Dynamic artifact/blob URL; next/image optimization is not suitable here. */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
...
function RichFileCard({
  ...
  if (isImage) {
    return (
      ...
      >
        {/* Dynamic artifact URL; keep native img to preserve current behavior. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={fileUrl}
          alt={file.filename}
